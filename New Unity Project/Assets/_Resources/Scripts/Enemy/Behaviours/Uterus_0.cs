...
public class Uterus_0 : MonoBehaviour
{
    Transform _t;
    ...
    public enum State { walk, attack, divide };
    ...
    float walkCounter;
    float waitCounter;
    [SerializeField] float divideTimer;
    ...
    void Update()
    {
        ...
        else if (curState == State.attack)
        {
            ...
            if (Vector2.SqrMagnitude(_t.position - player.transform.position) < 10f)
            {
        ...
        else if (curState == State.divide && !dividing)
        {
