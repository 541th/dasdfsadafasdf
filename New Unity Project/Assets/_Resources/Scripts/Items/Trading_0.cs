...
public class Trading_0 : MonoBehaviour
{
    ...
    private void Start()
    {
        for (int i = 0; i < 5; i++)
        {
            items[i] = Random.Range(0, 3) * 100 + Random.Range(0, 4) * 10 + Random.Range(0, 3);
    ...
    public void buy()
    {
        if (FindObjectOfType<UIManager>().subMonets(ItemDatabase.getItemById(items[choosenItemId]).value * 40))
        {
